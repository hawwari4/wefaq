...
للتجربة المحلية السريعة يمكن استخدام SQLite المدمجة. للإنتاج استخدم PostgreSQL (محلياً أو عبر Neon/Supabase مثلاً)، ثم:
...
يستخدم التطبيق SQLite تلقائياً في `backend/instance/wefaq.db`. وللاستخدام الإنتاجي يمكن إضافة `DATABASE_URL` حقيقي، مع ضبط `SECRET_KEY`. إعدادات `SMTP_*` اختيارية — بدونها يتم تخطي إرسال بريد الترحيب بصمت.
...
📍 يعمل على `http://localhost:5000` وينشئ الجداول وحساب المدير العام تلقائياً في قاعدة SQLite المحلية، أو في قاعدة `DATABASE_URL` عند ضبطها.

عند ضبط `WEFAQ_SEED_DEMO=true` في `backend/.env`، يضيف التطبيق مرة واحدة عشرة مرشحين تجريبيين مع ملفات وإجابات مكتملة: خمسة رجال (`DEMO001`–`DEMO005`) وخمس نساء (`DEMO006`–`DEMO010`).
