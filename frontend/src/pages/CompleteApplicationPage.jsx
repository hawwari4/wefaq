...
export default function CompleteApplicationPage() {
  ...
  function valid() {
    ...
    if (question.openNumber) return !textValidation(answers[`q${question.openNumber}`])
  ...
  function renderQuestion() {
    ...
    if (question.type === 'textarea') {
      ...
      return <><textarea autoFocus rows="6" value={(openKey ? answers[openKey] : value) || ''} onChange={onTextChange} placeholder={question.placeholder} className="w-full border-b-2 border-teal-100 bg-transparent py-3 text-xl leading-relaxed outline-none focus:border-gold-500" />{textError && <p className="mt-3 text-sm text-brick-500">{textError}</p>}</>
    }
