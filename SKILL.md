...
All commands are the ones the project actually supports.

### Backend
```bash
cd backend
cp .env.example .env          # Windows: copy .env.example .env
# then edit .env: DATABASE_URL is REQUIRED (app refuses to import without it)

python -m venv venv
source venv/bin/activate      # Windows PowerShell: .\venv\Scripts\Activate.ps1
pip install -r requirements.txt   # or, from the repo root: pip install -r requirements.txt

python app.py                 # http://localhost:5000 ; create_all() + ensure_schema_compatibility()
```
First run against an empty database: uncomment `seed_database()` in `app.py`'s `__main__`, run once to create the super admin, then re-comment it.

```bash
python test_system.py         # integration suite — currently broken, see §9.8
```
...
**There is no lint, format, type-check or unit-test command** in either `package.json` or the Python requirements. Do not invent `npm run lint` / `pytest` invocations — verify changes by running the app.

### Environment variables
| Var | Where | Purpose |
|---|---|---|
| `DATABASE_URL` | `backend/.env` | **Required.** PostgreSQL URI; `postgres://` is rewritten to `postgresql://` |
| `SECRET_KEY` | `backend/.env` | Flask secret (dev placeholder in `config.py`) |
| `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`SMTP_FROM` | `backend/.env` | Welcome email; incomplete → sending is skipped silently |
| `WEFAQ_DATA_DIR` / `WEFAQ_INSTANCE_DIR` / `WEFAQ_UPLOAD_DIR` | env | Override the data / instance / uploads paths |
| `WEFAQ_CORS_ORIGINS` | env | CORS origins (default `*`) |
| `WEFAQ_TESTING` | env | Testing flag |
| `WEFAQ_PUBLIC_BASE_URL` | env | Absolute base for generated photo URLs |
| `VITE_API_BASE_URL` | frontend env | Overrides `config.json`'s `apiBaseUrl` at build time |
